...
class Product:
    ...
    def impact_on_change_category(self, new_category: 'Category') -> tuple[set, set]:
        """E6: delta de atributos estaticos al mover el producto a new_category.
        Retorna (to_add, to_remove): lo que hay que implementar y lo que hay que quitar.
        Debe llamarse ANTES de mutar self.category."""
        current_static  = self._current_static_attrs()
        new_required    = {a for a in new_category.get_full_attr_set() if a.is_static}
        return new_required - current_static, current_static - new_required
